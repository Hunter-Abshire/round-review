...
- **Findings may carry `focus` shapes and the app draws them.** `parse_shapes` validates box/point/arrow in frame fractions with a label; anything outside the frame or unlabelled is dropped and never costs the finding. `renderer/overlay.ts` maps them to the *letterboxed picture* via `videoContentRect`, not the element, or they land wrong on any clip whose aspect differs from the window.
- **The review view is a video plus a tabbed sidebar.** Findings, coach and ask all render into `#side-body`; opening a finding replaces the list there. Never put review detail below the video again: that is what forced a scroll past the whole summary on every click. Playback uses `marker_selected` (highlight only) and clicks use `marker_opened` (highlight and open), so the video cannot hijack what is being read.
- **A finished review records what the clip was.** `identity.identity_index` takes the most common agent/map/side across windows so one odd window cannot rename a clip, and `played_at_from` reads the recorder's filename before falling back to mtime. Nothing is guessed: no review means no identity shown.
- **Settings are data, not a form.** `config.FIELDS` describes every editable setting (group, label, help, kind, bounds, unit, `choices_from`), and a test asserts every `Config` field is either described or in `EXCLUDED_FROM_UI`. Add a setting by adding a `FieldSpec`; the app renders it with no further change. `write_config` validates by loading what it is about to write, so a bad value never reaches the file.
- **The server holds config in a `ConfigHolder` and re-reads `holder.current` per request.** Never capture `config` in a closure in `app.py` or in `serve`'s job runners, or saved settings will not take effect until restart.
- **Questions retrieve; reviews do not.** `reference/` turns the bundled knowledge and the player's `notes_dir` into passages and ranks them with BM25 in pure Python (`reference/search.py`): no embedding model, no vector store, no network. Only `answer_question` uses it, because a review prompt is already token-bound and already gets the one agent and map in play. Tags (agent, map) multiply a score and so reorder matches but can never create one. Answers carry `sources` for auditing. There is no internet access anywhere in the package; the only outbound call is the Ollama transport.
