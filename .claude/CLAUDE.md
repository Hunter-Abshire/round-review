...
| `.venv/bin/round-review reference search "<question>"` | See which reference passages a question retrieves |
...
- **The HUD reads with no setup, and that is the contract.** The digit glyphs ship in `vision/templates/hud-digits.json` (learned from real footage; `normalize_glyph` maps to a fixed grid, so 720p-learned glyphs verifiably read 1080p and 1440p), the regions default to values measured off a real 16:9 recording, and `hud_threshold = -1` measures a cutoff per recording before anything else reads the HUD. `load_templates` merges what the player teaches on top of the bundled set rather than replacing it. Never reintroduce a state where a fresh install silently does no HUD checking: that was the old behaviour and it meant every new user got the model's unaided guesses.
- **The adaptive brightness cutoff is not good enough for the timer plate, and this is measured.** On real 720p footage `hud_threshold = -1` read the clock correctly in 6 of 16 frames; every fixed cutoff from 190 to 240 read all 16. The adaptive midpoint moves with whatever scenery sits behind the translucent plate, so glyphs merge on bright maps and split on dark ones, and `hud learn` then fails with the wrong glyph count. `hud calibrate --reads 25=1:39 ...` scores every cutoff against clocks the player read with their own eyes and returns the middle of the widest working run (`vision/calibrate.py`), because a cutoff that only just works stops working on the next map. Run it before `hud learn`, not after.
...
- **Rounds are found by scanning the clock, not by asking.** `vision/timeline.py` reads the round timer across the whole recording in one ffmpeg pass (`build_scan_args` puts `fps` first so crop and scale only run on kept frames) and splits at every barrier drop. Only a jump up to `ROUND_TIMER_MIN_S` counts: the buy phase (0:30) and the spike (0:45) also reset the clock upwards and must not read as new rounds. Round numbers are positions in the recording, never scoreboard numbers. `coverage = "rounds"` then plans windows where decisions live (round opening, round ending, and a lead-in window on each death) and falls back to full tiling when the scan finds nothing, so an untrained clock reader can never produce an empty review.
- **The rest of the HUD is read the same way, and all of it is off by default.** `vision/state.py` reads health and credits through the digit templates and decides ability availability by how much of each icon is lit. `vision/agent_icons.py` names the agent by matching learned icon sets across five moments. Every region setting is blank by default and every reader is a no-op when unconfigured: a HUD that has never been checked against real footage must degrade to unknown, never to a confident wrong answer. `hud state`, `hud deaths`, `hud rounds` and `hud learn-agent` exist so each one can be verified by eye before it is trusted. Measured values go into the coach prompt marked as read from pixels and overriding the model, and utility findings are dropped outright when every icon is dark.
- **Habits are tagged against previous matches.** `coaching/history.py` keeps `habits.json` beside the ledger (check counts and window count per reviewed match) and labels this match's checks new / repeat / improving / persistent against the last five. Compare rates, never counts, or a short Swiftplay looks like an improvement. A re-review replaces its own record rather than counting as a second match. The trend is a tie-breaker in `_rank`, not a new term in the documented weights. A missing or corrupt history file reads as no history; this sits on top of a review and must never fail one.
- **Feedback is the only measurement.** `feedback.py` appends thumbs to `feedback.jsonl` beside the ledger; the same clip + check + timestamp appended again is the player changing their mind, and `read_feedback(latest_only=True)` resolves it. `hit_rate` is how a model or prompt change gets compared against the last one instead of judged on a screenshot. A check dismissed past `MOSTLY_WRONG` over at least `MIN_RATINGS_TO_ACT` ratings sorts last in the report but is never removed: being unwelcome is not the same as being wrong.
- **The agent the player picked is an input, not a guess.** A told `PlayerContext.agent` is stated in the situation prompt and overwrites `situation.agent` when the model reads a different one, so the brief and the situation read never contradict each other in the coach prompt. `coaching/abilities.py` then drops any finding naming an ability the player's agent does not have (`ability_owners` from the briefs; single-word names need the capital because "seize" and "meddle" are ordinary English). This is the same argument as the clock: ability ownership is a fact in the data, so never let a 1280px portrait read decide it.
- **A decided round is not coached.** `_decided_round` suppresses the action categories (positioning, timing, trading, peeking, utility, info) when the player's side is `DECISIVE_ADVANTAGE` bodies ahead with no enemy on screen, because waiting out a 5v1 is correct play, not a positioning mistake. `enemies_alive` comes from the situation pass for this. Keep the threshold blunt: a 4v3 is still a round worth coaching and the counts come from a model reading a scoreboard.
